...
go_left_d1d2 = combined.loc[(combined['channel'] == 'go left') & (
    combined['type'] == 'd1d2')][['CC', 'CC No D2']]
go_right_d1d2 = combined.loc[(combined['channel'] == 'go right') & (
    combined['type'] == 'd1d2')][['CC', 'CC No D2']]
between_d1d1 = combined.loc[(combined['channel'] == 'between') & (
    combined['type'] == 'd1d1')][['CC', 'CC No D2']]
between_d2d2 = combined.loc[(combined['channel'] == 'between') & (
    combined['type'] == 'd2d2')][['CC', 'CC No D2']]
...
def plot_change(data, label, fn):
    ...
    sbn.violinplot(
        data, color=colors.colors[1], size=15., scale='width', ax=ax)
    pl.xticks([0, 1], ["with D2-D2", "w/o D2-D2"], rotation=70)
    pl.savefig(fn)


plot_change(go_left_d1d2, "CC (D1-D2) within 'turn left'",
            out_within_go_left_fn)
plot_change(go_right_d1d2, "CC (D1-D2) within 'turn right'",
            out_within_go_right_fn)
